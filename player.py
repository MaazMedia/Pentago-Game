...
class BoardGame:
    ...
    def create_board(self):
        # Create rows and quadrants
        for row in range(2):
            row_frame = tk.Frame(self.board_frame, bg='#2e2e2e')
            row_frame.pack(side=tk.TOP)
            for col in range(2):
                quad_frame = tk.Frame(row_frame, width=150, height=150, bg='#8B0000', borderwidth=1, relief=tk.SOLID)  # Dark red background for quadrants
                ...
                for r in range(3):
                    for c in range(3):
                        circle = tk.Button(quad_frame, width=6, height=3, bg='#98FB98', relief=tk.RAISED,  # Light green circles
                        ...
                        circle.token_color = None  # Track the token color
                ...
                # Add rotation buttons (initially hidden)
    ...
    def circle_click(self, row, col, quad_row, quad_col):
        if self.show_rotation:
            return  # Prevent clicking circles when rotation is enabled
        ...
        if not self.quadrants[quad_row][quad_col]['circles'][row][col].clicked:
            ...
            # Mark the button as clicked and update color
            self.quadrants[quad_row][quad_col]['circles'][row][col].clicked = True
            if self.current_player == 1:
                self.quadrants[quad_row][quad_col]['circles'][row][col].config(bg='#000000')  # Black for Player 1
                self.quadrants[quad_row][quad_col]['circles'][row][col].token_color = '#000000'
            else:
                self.quadrants[quad_row][quad_col]['circles'][row][col].config(bg='#ff0000')  # Red for Player 2
    ...
    def rotate_quadrant(self, row, col, right=True):
        """Rotate the specified quadrant 90 degrees."""
        quad = self.quadrants[row][col]
        circles = quad['circles']
        
        # Create a new 3x3 matrix for rotated quadrant
        ...
        # Apply the rotated quadrant back to the UI
        for r in range(3):
            for c in range(3):
                color = rotated_tokens[r][c]
                clicked = rotated_clicked[r][c]
                circle = quad['circles'][r][c]
                circle.config(bg=color if color else '#98FB98')  # Light green for unclicked circles
                circle.token_color = color
                circle.clicked = clicked
        
        # Hide rotation buttons after rotation
        self.hide_rotation_buttons()

        # Switch the player after rotation
        self.current_player = 1 if self.current_player == 2 else 2
        self.player_turn_label.config(text=f"Player {self.current_player}'s Turn")
        
        # Check if someone has won after the rotation
        if self.check_winner():
            print(f"Player {self.current_player} wins!")
            self.reset_board()
    ...
    def check_winner(self):
        def has_five_in_a_row(tokens):
            """Check if there are 5 consecutive tokens of the same color in the list."""
            count = 0
            last_token = None
            for token in tokens:
    ...
    def hover_effect(self, circle):
        if not circle.clicked:
            circle.config(bg='#87CEEB')  # Light blue on hover

    def leave_effect(self, circle):
        if not circle.clicked:
            circle.config(bg='#98FB98')  # Light green when not hovered
        ...
        for row in range(2):
            for col in range(2):
                ...
                for r in range(3):
                    for c in range(3):
                        circle = quad['circles'][r][c]
                        circle.config(bg='#98FB98')  # Light green for reset
                        circle.clicked = False
